...
@export var aiming_fov_change : float
@export var aiming_offset_x = 0.01
@export var aiming_offset_y = -0.01
...
func _ready():
	DefaultPosition = get_parent().position
...
func is_one_handed():
	if get_parent() == player.LeftHand:
		if player.RightHand.get_child_count() > 0 or !is_two_handed:
			return true
		elif is_two_handed:
			return false
		else:
...
func _process(delta: float) -> void:
	...
	if aiming:
		player.camera.fov = lerpf(player.camera.fov, aiming_fov_change, heftiness_lerp * delta)
		if get_parent() == player.LeftHand:
			if is_one_handed():
				player.crosshair.show()
				get_parent().position = lerp(get_parent().position, aiming_position + Vector3(-aiming_offset_x,aiming_offset_y,0),  heftiness_lerp * delta)
			else:
		...
		player.crosshair.show()
		player.camera.fov = lerpf(player.camera.fov, 100, heftiness_lerp * delta)
